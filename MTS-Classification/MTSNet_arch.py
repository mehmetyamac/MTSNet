...
def calculate_conv_Mac(C_in, C_out, H, W, K, groups=1):
    ...
    return C_in * C_out * H * W * (K ** 2) / groups # 32 channel,             
...
class MultiScale(nn.Module):
    ...
    def __init__(self, in_channels=30, out_channels=30, T=3, window_scales=[16, 32, 64, 128],
                 ...
                 auto_differentiate: bool = True, use_einsum: bool = False, tensor_sum_type="efficient", drop_block_p=0.1):
    ...
    #def forward(self, x: torch.Tensor): If you do not want to use Jit you can use this part. 
    #    y = self.MOs[0](x)
...
class MTSNet(nn.Module):
    def __init__(self, N, NB, in_channels=3, groups=3, num_heads=5, T=3,
                 ...
                 alpha_decay=0.1, beta_decay=1, reduction_list = [], dwconv_usage=False, drop_block_p=0.1):
