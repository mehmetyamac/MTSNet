...
class GaussianDenoisingDataset(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(GaussianDenoisingDataset, self).__init__()
        self.opt = opt
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = []
            for entry in scandir(self.gt_folder):
                if entry.is_file():
                    gt_path = os.path.join(self.gt_folder, entry.name)
                    self.paths.append({'gt_path': gt_path})
