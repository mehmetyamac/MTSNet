...
def parse_options(is_train=True):
    ...
    if args.launcher == 'none':
        ...
        # Debugging: print rank and world size
        print(f"Rank {dist.get_rank()} out of {dist.get_world_size()} in total.", flush=True)

    # Get rank and world_size from the distributed initialization
...
def init_loggers(opt):
    ...
    # Log environment info and options
    logger.info(get_env_info())
    logger.info(dict2str(opt))

    # Initialize wandb logger if needed
    ...
    tb_logger = None
    # Set a custom log directory for TensorBoard logger
    if opt['logger'].get('use_tb_logger') and 'debug' not in opt['name']:
        # Modify this line to specify your desired log directory
...
def main():
    ...
    if resume_state is None:
        ...
        if opt['logger'].get('use_tb_logger') and 'debug' not in opt['name'] and opt['rank'] == 0:
            mkdir_and_rename(osp.join('..', '..', '..', 'logs', 'tb_logger', opt['name']))
    ...
    # Ensure total_iters is set properly
    ...
    if not iters or len(iters) == 0:
        raise ValueError("Iters must be specified in the configuration.")
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):
                logger.info(f'Running validation at iteration {current_iter}.')  # Log validation start

                # Debugging: You can check specific conditions like RGB conversion, etc.
                rgb2bgr = opt['val'].get('rgb2bgr', True)
                use_image = opt['val'].get('use_image', True)

                logger.info(f'Validation settings: rgb2bgr={rgb2bgr}, use_image={use_image}')
