...
def mkdir_and_rename(path):
    ...
    """
    if osp.exists(path):
        # Prepare new name with a timestamp
        ...
        try:
            ...
            print(f'Error renaming {path} to {new_name}: {e}', flush=True)
            return  # Exit the function if renaming fails

    # Attempt to create the directory
    try:
        os.makedirs(path, exist_ok=True)
        print(f'Created directory: {path}', flush=True)
    except Exception as e:
        print(f'Error creating directory {path}: {e}', flush=True)
